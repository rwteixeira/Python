...
ContMeses = len(Resultado)

for MesCol in range(1, ContMeses + 1):
    guia_resultados.cell(1, 3 + MesCol).value = "Mês " + str(Resultado[MesCol - 1])
...
for linha in range(5, Ulinha + 1):
    ...
    dia = guia_contas_receber.cell(linha, 8).value
    mês = guia_contas_receber.cell(linha, 9).value
    ...
    if situacao == "Atrasado":
        if juros == 0:
            ...
            guia_resultados.cell(2, 14).value = Resultado[0]
        else:
            mora = 1.3
            ...
            guia_resultados.cell(3, 14).value = Resultado[1]
